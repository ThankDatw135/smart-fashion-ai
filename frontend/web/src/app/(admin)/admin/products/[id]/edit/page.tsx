...
export default function AdminEditProductPage() {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      const payload = {
        ...
        variants: variants.map((v: any) => ({ ...v, stock: Number(v.stock), price: Number(v.price) }))
      };
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Đang tải dữ liệu sản phẩm...</div>;
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Cập nhật thông tin sản phẩm <span className="font-semibold text-foreground">{productId}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 rounded-3xl border bg-card text-card-foreground shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea {...register("description")} placeholder="Viết mô tả chi tiết..." className="min-h-[200px]" />
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {images.map((src, index) => (
                <div key={index} className="relative aspect-square rounded-xl border bg-muted overflow-hidden group">
                  <Image src={src.includes("http") || src.startsWith("/") ? src : `/${src}`} alt="Preview" fill className="object-cover" />
                  <button type="button" onClick={() => removeImage(index)} className="absolute top-2 right-2 p-1.5 bg-background/80 backdrop-blur rounded-full text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ))}
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y">
                  {variants.map((variant) => (
                    <tr key={variant.id}>
                      <td className="p-3"><Input value={variant.size} onChange={(e) => updateVariant(variant.id, "size", e.target.value)} className="w-20 h-8" placeholder="Size" /></td>
                      <td className="p-3"><Input value={variant.color} onChange={(e) => updateVariant(variant.id, "color", e.target.value)} className="w-24 h-8" placeholder="Màu sắc" /></td>
                      <td className="p-3"><Input value={variant.price} onChange={(e) => updateVariant(variant.id, "price", e.target.value)} type="number" className="w-28 h-8" placeholder="Giá" /></td>
                      <td className="p-3"><Input value={variant.stock} onChange={(e) => updateVariant(variant.id, "stock", e.target.value)} type="number" className="w-20 h-8" placeholder="SL" /></td>
                      <td className="p-3"><Input value={variant.sku} onChange={(e) => updateVariant(variant.id, "sku", e.target.value)} className="h-8" placeholder="SKU" /></td>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Button type="button" variant="outline" className="w-full text-muted-foreground" onClick={() => router.back()}>Bỏ qua thay đổi</Button>
            <Button type="button" variant="ghost" className="w-full text-destructive hover:text-destructive hover:bg-destructive/10 mt-4" onClick={() => router.push("/admin/products")}>
              Xóa Sản Phẩm Này
            </Button>
          </div>
        </div>
