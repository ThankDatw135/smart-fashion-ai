...
export default function ContactClient({ content }: { content: any }) {
  ...
  const contactInfo = [
    { icon: MapPin, title: "Trụ sở chính", detail: content.address || "123 Cầu Giấy, Hà Nội, Việt Nam" },
    { icon: Phone, title: "Hotline CSKH", detail: content.phone || "1900 1234 (8:00 - 22:00)" },
    { icon: Mail, title: "Email hỗ trợ", detail: content.email || "support@antigravity.store" },
    { icon: Clock, title: "Giờ làm việc", detail: content.workingHours || "T2 - CN: 8:00 am - 10:00 pm" },
  ];
  ...
  return (
    <div className="container py-16 lg:py-24">
      <div className="text-center max-w-2xl mx-auto mb-16">
        <h1 className="text-4xl md:text-5xl font-heading font-bold mb-4">{content.title || "Liên hệ với chúng tôi"}</h1>
        <p className="text-lg text-muted-foreground">
          {content.description || "Bạn có thắc mắc về đơn hàng, sản phẩm hay chương trình khuyến mãi? Hãy để lại lời nhắn, đội ngũ tư vấn sẽ hỗ trợ bạn sớm nhất."}
        </p>
      </div>
        ...
        <div className="space-y-10">
          ...
          <div className="bg-muted/50 p-6 rounded-2xl border">
            ...
            <Button 
              variant="outline" 
              className="w-full"
              onClick={() => window.location.href = `mailto:${content.partnerEmail || "partner@antigravity.store"}`}
            >
              {content.partnerEmail || "partner@antigravity.store"}
            </Button>
          </div>
        </div>
